...
export class ParserFactoryService {
  ...
  async getParser(
    ...
    console.log(`[ParserFactory] No suitable parser found for bank: ${bankName}, fileType: ${fileType}`);
    return null;
  }

  async detectBankAndFormat(filePath: string, fileType: FileType): Promise<{
    bankName: BankName;
    formatVersion?: string;
  }> {
    console.log(`[ParserFactory] Detecting bank and format for file: ${filePath}, type: ${fileType}`);
    ...
    for (const parser of this.parsers) {
      ...
      // Check Kaspi first
      if (parser instanceof KaspiParser && await parser.canParse(BankName.KASPI, fileType, filePath)) {
        console.log(`[ParserFactory] Detected: Kaspi Bank`);
        return { bankName: BankName.KASPI };
      }
