...
- ✅ Frontend and Backend run from a single Railway service
- ✅ Both accessible from one domain (no CORS issues)
- ✅ Automatic static file serving for the frontend
- ✅ API accessible at `/api/v1` on the same domain

## 📦 Files Modified/Created

### Core Application Files
1. **`Dockerfile`** (root)
   - Builds Next.js frontend → NestJS backend
...
Railway.app
│
├─ Single Service: parse-ledger-production
│  │
│  ├─ Port 3001
│  │
│  ├─ Express Server (NestJS)
│  │  │
│  │  ├─ GET  /                    → Next.js Frontend (index.html)
│  │  ├─ GET  /api/v1/*            → NestJS API Routes
│  │  ├─ GET  /api/docs            → Swagger Documentation
│  │  ├─ GET  /*.js, /*.css, etc   → Frontend Static Assets
