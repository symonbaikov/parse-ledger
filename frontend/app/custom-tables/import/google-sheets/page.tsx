...
export default function GoogleSheetsImportPage() {
  ...
  const canPreview = Boolean(
    googleSheetId && selectedConnection?.oauthConnected !== false,
  );
  const canCommit = Boolean(
    preview && tableName.trim() && columns.some((c) => c.include),
  );
  ...
  const loadConnections = async () => {
    ...
    try {
      ...
      const items: GoogleSheetConnection[] =
  ...
  useEffect(() => {
    ...
    setTableName(
      (prev) =>
        prev || selectedConnection.sheetName || t.defaults.tableName.value,
    );
  }, [selectedConnection]);
  ...
  const handlePreview = async () => {
    ...
    try {
      const response = await apiClient.post(
        "/custom-tables/import/google-sheets/preview",
        {
          googleSheetId,
      ...
      const message =
        error?.response?.data?.message || t.toasts.previewFailed.value;
      toast.error(message);
    } finally {
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      const message =
        error?.response?.data?.message || t.toasts.importFailed.value;
      toast.error(message);
    } finally {
      setCommitting(false);
    }
    ...
    const poll = async () => {
      try {
        const response = await apiClient.get(
          `/custom-tables/import/jobs/${jobId}`,
        );
        const payload = response.data?.data || response.data;
        if (cancelled) return;
              ...
              >
                ...
                {connections.map((c) => (
                  <option
                    key={c.id}
                    value={c.id}
                    disabled={c.oauthConnected === false}
                  >
                    {c.sheetName}
                    {c.oauthConnected === false
                      ? t.source.oauthNeededSuffix.value
                      : ""}
                  </option>
                ))}
              </select>
              ...
              <div className="mt-3 rounded-lg border border-gray-200 bg-white p-3">
                ...
                <div className="mt-2 text-xs text-gray-600">
                  ...
                  <span className="font-medium">
                    {jobStatus || t.result.dash.value}
                  </span>{" "}
                  {jobStage ? (
                    <span className="text-gray-500">({jobStage})</span>
                  ) : null}
                </div>
                {jobError ? (
                      ...
                      >
                        ...
                        <td className="px-3 py-2">
                          ...
                          >
                            ...
                            <option value="number">
                              {t.columns.types.number}
                            </option>
                            <option value="date">{t.columns.types.date}</option>
                            <option value="boolean">
                              {t.columns.types.boolean}
                            </option>
                            <option value="select">
                              {t.columns.types.select}
                            </option>
                            <option value="multi_select">
                              {t.columns.types.multiSelect}
                            </option>
                          </select>
                        </td>
                      </tr>
