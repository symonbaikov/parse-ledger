...
const content = {
  ...
  content: {
    toasts: {
      ...
      previewFailed: t({ ru: 'Не удалось получить превью', en: 'Failed to get preview', kk: 'Превью алу мүмкін болмады' }),
      importStartFailed: t({ ru: 'Не удалось запустить импорт', en: 'Failed to start import', kk: 'Импортты бастау мүмкін болмады' }),
      importStarted: t({ ru: 'Импорт запущен', en: 'Import started', kk: 'Импорт басталды' }),
      importFailed: t({ ru: 'Не удалось выполнить импорт', en: 'Import failed', kk: 'Импорт орындалмады' }),
      importDone: t({ ru: 'Импорт завершён', en: 'Import completed', kk: 'Импорт аяқталды' }),
      importError: t({ ru: 'Импорт завершился с ошибкой', en: 'Import finished with an error', kk: 'Импорт қателікпен аяқталды' }),
    },
    defaults: {
      tableName: t({ ru: 'Импорт из Google Sheets', en: 'Import from Google Sheets', kk: 'Google Sheets-тен импорт' }),
    },
    auth: {
      loginRequired: t({
    ...
    header: {
      title: t({ ru: 'Импорт из Google Sheets', en: 'Import from Google Sheets', kk: 'Google Sheets-тен импорт' }),
      subtitle: t({
    ...
    source: {
      ...
      connectionLabel: t({ ru: 'Подключение Google Sheet', en: 'Google Sheet connection', kk: 'Google Sheet қосылымы' }),
      ...
      worksheetPlaceholder: t({ ru: 'Например: Реестр платежей', en: 'e.g. Payments registry', kk: 'Мысалы: Төлемдер тізілімі' }),
      worksheetHelp: t({
      ...
      rangeLabel: t({ ru: 'Range (опционально)', en: 'Range (optional)', kk: 'Range (міндетті емес)' }),
      rangePlaceholder: t({ ru: 'Например: A1:Z200', en: 'e.g. A1:Z200', kk: 'Мысалы: A1:Z200' }),
      headerOffsetLabel: t({ ru: 'Header row offset', en: 'Header row offset', kk: 'Header row offset' }),
      headerOffsetHelp: t({ ru: '0 = первая строка used range', en: '0 = first row of used range', kk: '0 = used range бірінші жолы' }),
      ...
      loadingConnections: t({ ru: 'Загружаем подключения…', en: 'Loading connections…', kk: 'Қосылымдар жүктелуде…' }),
    ...
    result: {
      ...
      tableNamePlaceholder: t({ ru: 'Например: Реестр платежей', en: 'e.g. Payments registry', kk: 'Мысалы: Төлемдер тізілімі' }),
      descriptionLabel: t({ ru: 'Описание (опционально)', en: 'Description (optional)', kk: 'Сипаттама (міндетті емес)' }),
      categoryLabel: t({ ru: 'Категория (иконка/цвет)', en: 'Category (icon/color)', kk: 'Санат (иконка/түс)' }),
      ...
      importDataCheckbox: t({ ru: 'Импортировать данные (кроме заголовка)', en: 'Import data (except header)', kk: 'Деректерді импорттау (тақырыптан басқа)' }),
      importButton: t({ ru: 'Импортировать', en: 'Import', kk: 'Импорттау' }),
      importRunning: t({ ru: 'Импорт выполняется…', en: 'Import is running…', kk: 'Импорт орындалуда…' }),
      progressTitle: t({ ru: 'Прогресс', en: 'Progress', kk: 'Үдеріс' }),
      statusLabel: t({ ru: 'Статус', en: 'Status', kk: 'Күйі' }),
      needPreviewHint: t({ ru: 'Сначала сделайте превью.', en: 'Make a preview first.', kk: 'Алдымен превью жасаңыз.' }),
      dash: t({ ru: '—', en: '—', kk: '—' }),
    },
    preview: {
      ...
      hint: t({ ru: 'Выберите подключение и нажмите “Сделать превью”.', en: 'Select a connection and click “Make preview”.', kk: 'Қосылымды таңдап, “Превью жасау” басыңыз.' }),
    ...
    columns: {
      ...
      appearAfterPreview: t({ ru: 'Колонки появятся после превью.', en: 'Columns will appear after preview.', kk: 'Бағандар превьюдан кейін пайда болады.' }),
      tableHeaders: {
